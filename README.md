# 도다 (DODA)
도다 – 닿지 못했던 곳까지 함께 도달하다
정보 장벽을 낮추는 콘텐츠 플랫폼

>돋다 : 감정, 표현, 사회성이 서서히 자라나는 의미

>도달하다 : 세상에 연결되고, 사회에 포함되는 과정
...
- 구매한 도메인 winnerteam.store에 연결하여 접근 가능
...
|  배포 주소 | [배포 주소](https://winnerteam.store/api/news/latest) |
...
프론트엔드는 React 기반 SPA(Single Page Application)로 개발되었으며, 백엔드의 REST API와 HTTP 통신을 통해 데이터 연동을 수행합니다.

* **통신 방식**: Axios를 활용한 RESTful API 호출
* **인증 처리**: JWT 토큰을 로그인 시 발급받아, 이후 모든 인증이 필요한 요청에 `Authorization` 헤더(`Bearer <토큰>`)로 포함
* **CORS 문제 해결**: 백엔드(Spring Boot)에서 CORS 설정을 통해 프론트의 요청을 허용 (도메인 패턴 `*` 사용)
* **배포 연동**:

  * 프론트는 Vercel에 배포, 환경 변수로 `https://winnerteam.store` 주소의 백엔드 API 서버를 설정
  * 백엔드는 AWS EC2에 Docker 기반으로 배포되어 `https://winnerteam.store` 도메인으로 접근 가능
* **API 연동 예시**:

  * `/api/users/signup`, `/api/users/login` → 회원가입 및 로그인 처리
  * `/api/news/latest` → 홈 화면 뉴스 카드 조회
  * `/api/welfare` → 복지 리스트 출력
  * `/api/welfare/{id}` → 복지 상세 정보
  * `/api/welfare/{id}/bookmark` → 복지 북마크 저장/해제
  * `/api/welfare/me/bookmark` → 마이페이지에서 북마크한 복지 목록 조회
